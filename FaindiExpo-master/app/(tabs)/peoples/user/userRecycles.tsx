...
interface UserRecyclesPageProps {
  products: ProductProps[] | [];
}

export default function UserRecyclesPage({ products }: UserRecyclesPageProps) {
  const navigation = useRouter();
  const { userId } =  useAuth();

  const recycleProducts = React.useMemo(() => {
    return products?.filter((product) => product.is_recycle);
  }, [products]);
  ...
  return (
    <View style={styles.container}>
              ...
              {prodcuts.map((product, _index) => (
                ...
                >
                  <View style={styles.likeBox}>
                    <AntDesign
                      style={[styles.likeHeart, checkIsLike(product, userId) && {
                        color: "#edaeff",
                      },]}
                      name="heart"
                      size={36}
                  ...
                  <Text style={styles.productPrice}>{product.reduced_price>0? product.reduced_price: product.price} {getCurrencySymbol(product.currency)}</Text>
                </Pressable>
              ))}
              {prodcuts.length === 2 && (
      ...
      {products.length === 0 && <View><Text>No user's recycled products</Text></View>}
    </View>
  );
}
